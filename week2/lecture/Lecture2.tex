...
\begin{itemize}
	\item \textbf{Definition:} The text segment is a region of a program's memory that contains the compiled machine code of the program itself.
...
\begin{itemize}
	...
	\item Static data can be further divided into:
	\begin{itemize}
...
\noindent
The heap is a region of memory used for dynamic memory allocation. It is managed manually by the programmer or automatically by a memory manager (in the case of garbage-collected languages).

Memory in the heap is allocated and freed during runtime, allowing flexible memory usage based on the program's needs. Functions like new and malloc allocate memory on the heap, while delete and free deallocate it.

It is the programmer’s responsibility to manage heap memory. Failure to deallocate memory leads to memory leaks, where memory that is no longer in use is not freed.

Over time, the heap can become fragmented, meaning that free memory is scattered in smaller chunks, which can lead to inefficient use of available memory.
